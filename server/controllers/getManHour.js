...
module.exports = async ctx => {
	...
	let prevres = await DB('signin_fore').select(DB.raw('*')).where(DB.raw(" yearweek(DATE_FORMAT(`update_time`, '%Y-%m-%d'),5) = yearweek(DATE_SUB(NOW(),INTERVAL 7 DAY),5)")).where({ name: name, studentNum: studentNum })
